-- ============================================================
-- BOX MANAGER — Esquema completo unificado
-- Versión 2.0 — incluye módulo Clase en vivo
...
-- 4. Listo — todas las tablas se crean en orden correcto
-- ============================================================
-- Este archivo usa "IF NOT EXISTS" en todas partes,
-- lo que significa que es seguro ejecutarlo varias veces
-- sin duplicar ni borrar datos existentes.
-- ============================================================
...
-- ============================================================
-- SESIONES (depende de clases — va después)
-- ============================================================
...
-- Política general: solo usuarios autenticados pueden acceder
do $$
declare
  t text;
  tables text[] := array[
    'atletas','accesos','pagos','rutinas','clases','clase_atletas',
    'clase_fases','clase_progreso','clase_estado_timer','sesiones',
    'mediciones','productos','ventas','gastos','configuracion_box','politicas_box'
  ];
begin
  foreach t in array tables loop
    execute format(
      'create policy if not exists "acceso_autenticado_%s" on %s for all using (auth.role() = ''authenticated'')',
      t, t
    );
  end loop;
end $$;
...
-- ============================================================
-- TIEMPO REAL — para Clase en vivo
-- ============================================================
...
-- Trigger helper: actualizar updated_at automáticamente
...
-- Progreso de fase (upsert sin conflictos)
...
-- Finalizar clase y guardar historial de atletas
...
-- Días para que venza el pago de un atleta
...
drop trigger if exists clases_updated_at       on clases;
drop trigger if exists clase_progreso_updated  on clase_progreso;
...
values (
  E'REGLAMENTO INTERNO DEL BOX\n\n1. El BOX se reserva el derecho de admisión.\n2. Los atletas deben respetar los equipos y el espacio.\n3. Horario de atención: lunes a sábado. Sin servicio los domingos.\n4. Se requiere indumentaria deportiva apropiada.',
  E'CONSENTIMIENTO INFORMADO\n\nDeclaro que mi estado de salud me permite realizar actividad física de alta intensidad.\nHe informado al coach sobre cualquier condición médica relevante.\nEntiendo y acepto los riesgos inherentes al CrossFit.'
)
on conflict do nothing;

-- ============================================================
-- VERIFICACIÓN FINAL
-- Muestra cuántas tablas se crearon correctamente
-- ============================================================
select
  table_name,
  'OK' as estado
...
where table_schema = 'public'
  and table_name in (
    'configuracion_box','usuarios_sistema','planes','atletas',
    'grupos','grupo_miembros','pagos','accesos','rutinas',
    'clases','clase_atletas','clase_fases','clase_progreso',
    'clase_estado_timer','sesiones','mediciones',
    'productos','ventas','gastos','politicas_box'
  )
order by table_name;
